...
Public Class Scot_stack2
    ...
    Public Sub createPage(ByVal sArea As String, ByVal sPipe As String, ByVal sPageType As String)
        ...
        If sPageType = "Scot_stack2" Then
            Call createScock_stack()
        End If

    End Sub
    Private Sub createScock_stack()
        ...
        sTags(0) = "AI070988_2.PV"
        ' sTags(1) = "AI070988A_1.PV"
        sTags(1) = "AI070989_2.PV"
        sTags(2) = "AI070990_2.PV"
        sTags(3) = "AI070991_2.PV"
        ...
        For i = 0 To 3
            If OSIsoft.AF.PI.PIPoint.TryFindPIPoint(srvAF, sTags(i), sinusoid) Then
                '  avalues = sinusoid.RecordedValues(timerange1, Data.AFBoundaryType.Interpolated, "", True)

                returnValue = sinusoid.Summary(timerange1, Data.AFSummaryTypes.All, Data.AFCalculationBasis.TimeWeighted, Data.AFTimestampCalculation.Auto)

                itemp = 0
                If sTagDesc(i) = "O2" Or sTagDesc(i) = "Hum" Then
                    itemp = 1
                End If

                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMin" & sTagDesc(i) & "_EPB")
                mylbl.Content = FormatNumber(returnValue.Values(4).Value, itemp)
                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMax" & sTagDesc(i) & "_EPB")
                mylbl.Content = FormatNumber(returnValue.Values(5).Value, itemp)
                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMean" & sTagDesc(i) & "_EPB")
                mylbl.Content = FormatNumber(returnValue.Values(3).Value, itemp)

            End If
        Next

        sTags(0) = "XHN_GAS-PLNT_TRN-2_STACK_SO2-ANLYZR-1.PV"
        sTags(1) = "AIC070906_2.DACA.PV"
        ...
        For i = 0 To 1
            If OSIsoft.AF.PI.PIPoint.TryFindPIPoint(srvAF, sTags(i), sinusoid) Then
                ...
                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMean" & sTagDesc(i) & "_CDB")
                If returnValue.Values(2).IsGood Then
                    mylbl.Content = FormatNumber(returnValue.Values(3).Value, itemp)
                Else
                    mylbl.Content = "ERR"
                End If

                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMin" & sTagDesc(i) & "_CDB")
                If returnValue.Values(3).IsGood Then
                    mylbl.Content = FormatNumber(returnValue.Values(4).Value, itemp)
                Else
                    mylbl.Content = "ERR"
                End If

                mylbl = LogicalTreeHelper.FindLogicalNode(mycanvas, "lblMax" & sTagDesc(i) & "_CDB")
                If returnValue.Values(4).IsGood Then
                    mylbl.Content = FormatNumber(returnValue.Values(5).Value, itemp)
                Else
                    mylbl.Content = "ERR"
                End If
        ...
        pi_col_xls_tags(1) = "XHN_GAS-PLNT_TRN-2_STACK_SO2-ANLYZR-1.PV"
        pi_col_xls_tags(2) = "AI070988_2.PV"
    ...
    Private Sub refreshChart(ByVal sChartName As String, ByVal ipt As Integer)
        ...
        'Height="273"  Width="658" Canvas.Left="184" Canvas.Top="31"
        ileft = 184
        iTop = 31
        iWidth = 658
        iHeight = 273
        ...
        sSeriesNames(1) = "CDB S02"
        sSeriesNames(2) = "EPB SO2"
    ...
    Private Sub Scot_stack2_SizeChanged(sender As Object, e As SizeChangedEventArgs) Handles Me.SizeChanged
    ...
    Private Sub Scot_stack2_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
