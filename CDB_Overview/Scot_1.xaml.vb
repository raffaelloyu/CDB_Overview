...
Public Class Scot_1
    ...
    Private pi_col_xls_tags(10) As String
    ...
    Public Sub New()
        ...
        ' imgTrend.AddHandler(PreviewMouseDownEvent, New RoutedEventHandler(AddressOf openTrends))
    ...
    Private Sub mycanvas_MouseMove(sender As Object, e As MouseEventArgs) Handles mycanvas.MouseMove
        ...
        End If
    End Sub
    Private Sub btnScot_profile_Click(sender As Object, e As RoutedEventArgs) Handles btnScot_profile.Click
        Dim newWinThread As New Thread(AddressOf startScotprofile)
        newWinThread.IsBackground = True
        newWinThread.SetApartmentState(ApartmentState.STA)
        newWinThread.Start()
    End Sub
    Public Sub startScotprofile()
        Call writeToLog("ScotProfile")
        Dim newdia As New Reactor
        newdia.Show()
    ...
    Public Sub createPage(ByVal sArea As String, ByVal sPipe As String, ByVal sPageType As String)
        ...
        Dim stemp As String
        Dim sType As String
        Dim sScale, sEqu As String

        Dim sEX As String
        Dim sTag As String
        Dim oTransform As RotateTransform
                ...
                Try
                    ...
                    If Not (config_aa.SelectSingleNode("variables/input[@name='" & sName & "']") Is Nothing) Then
                        ...
                        If stemp = "animation" Then
                            '  oWeelAuto = New DoubleAnimation
                            '   If sTagName = "clockwise" Then
                            '  oWeelAuto.From = 360
                            'ElseIf sTagName = "counterclock" Then
                            '  oWeelAuto.From = -360
                            'End If
                            '   oTransform = New RotateTransform
                            '  newimage.RenderTransform = oTransform
                        End If
        ...
        If sPageType = "Scot_1" Then
            Call createScot()
        End If

    End Sub
    Private Sub createScot()
        Dim sTags(1) As String

        sTags(0) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_H2-ANLYZR-1.PV"
        sTags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_H2-ANLYZR-2.PV"
        ...
        pi_col_xls_tags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_H2-ANLYZR-1.PV"
        pi_col_xls_tags(2) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_H2-ANLYZR-2.PV"
        ...
        sTags(0) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_OUTLET-PH-MONITOR-1.PV"
        sTags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_OUTLET-PH-MONITOR-2.PV"
        ...
        pi_col_xls_tags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_OUTLET-PH-MONITOR-1.PV"
        pi_col_xls_tags(2) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_OUTLET-PH-MONITOR-2.PV"
        ...
        sTags(0) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_DP.PV"
        sTags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_CHMBR-DP.PV"
        sTags(2) = "XHN_GAS-PLNT_TRN-1_SCOT-ABSORBR_CHMBR-DP.PV"
        sTags(3) = "XHN_GAS-PLNT_TRN-1_SCOT-REACTR_DP.PV"
        sTags(4) = "XHN_GAS-PLNT_TRN-1_SOUR-H2O-STRIP_CHMBR-DP.PV"
        '  sTags(4) = "XHN_GS-B_ACID-GAS-COMP-1_STG-5-DP.PV"

        foundPoints = OSIsoft.AF.PI.PIPoint.FindPIPoints(srvAF, sTags)
        pts(3) = New OSIsoft.AF.PI.PIPointList(foundPoints)

        ''  ReDim pi_col_xls_tags(4)

        pi_col_xls_tags(1) = "XHN_GAS-PLNT_TRN-1_QNCH-H2O-FLTR_DP.PV"
        pi_col_xls_tags(2) = "XHN_GAS-PLNT_TRN-1_QNCH-COLUMN_CHMBR-DP.PV"
        pi_col_xls_tags(3) = "XHN_GAS-PLNT_TRN-1_SCOT-ABSORBR_CHMBR-DP.PV"
        pi_col_xls_tags(4) = "XHN_GAS-PLNT_TRN-1_SCOT-REACTR_DP.PV"
        pi_col_xls_tags(5) = "XHN_GAS-PLNT_TRN-1_SOUR-H2O-STRIP_CHMBR-DP.PV"
        '  pi_col_xls_tags(5) = "XHN_GS-B_ACID-GAS-COMP-1_STG-5-DP.PV"
    ...
    Private Sub refreshChart(ByVal sChartName As String, ByVal ipt As Integer)
        ...
        sTime_end = Now
        sTime_start = DateAdd(DateInterval.Hour, -4, sTime_end)
        ...
        ' If False Then
        Call getChartData(1, strXML, iops, iPoints, pts(ipt))

        'Height="253"  Width="620" Canvas.Left="214" Canvas.Top="46

        'Height="174" Canvas.Left="1275" Stroke="Gray" Canvas.Top="56" Width="263"
        ileft = 1600
        iTop = 56
        iWidth = 275
        iHeight = 174
        ...
        If sChartName = "chart_flows_lean" Then
            ...
            'Height="174" Canvas.Left="1275" Stroke="Gray" Canvas.Top="243" Width="263"
            ...
            sSeriesNames(1) = "pH Monitor 1"
            sSeriesNames(2) = "pH Monitor 2"
            '  sSeriesNames(3) = "to SCOT"
        ElseIf sChartName = "chart_flows_acid" Then
            'Height="210" Canvas.Left="1122" Canvas.Top="695" Width="550"
            'Height="375" Canvas.Left="1275" Stroke="Gray" Canvas.Top="430" Width="263"
            ...
            sSeriesNames(1) = "Quench Water Filter"
            sSeriesNames(2) = "Quench Column"
            sSeriesNames(3) = "SCOT Absorber"
            sSeriesNames(4) = "SCOT Reactor"
            sSeriesNames(5) = "SW Stripper"
        End If
        ...
        ' cha.AxesX(0).IntervalType = IntervalTypes.Minutes
        '   cha.AxesX(0).IntervalType = 30
    ...
    End Sub
    Private Sub getTrend(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs)
        Dim sEX, sVar, sType, sScale, sQue As String
        ...
        Else
            ...
            If sender.Tag <> "" Then
                ...
                strXML = "<variables>"
                strXML = strXML & "<input trend='yes' tag='" & sender.Tag & "' type='" & sType & "' var='" & sVar & "' equ='" & sQue & "' ext='" & sEX & "' scale='" & sScale & "'><value/><time/></input>"
                strXML = strXML & "</variables>"
            ...
            ElseIf iGatherType = 1 Then
                ...
                For i = 0 To iops - 1
                    If OSIsoft.AF.PI.PIPoint.TryFindPIPoint(srvAF, pi_col_xls_tags(i + 1), sinusoid) Then
                        ...
                        For Each avalue In avalues
                            If avalue.IsGood Then
                                rValues_as(i, ii) = avalue.Value ' FormatNumber(avalue.Value, 0)
                            Else
                                rValues_as(i, ii) = -9
                            End If
                            sDates_as(i, ii) = avalue.Timestamp.LocalTime.ToString
    ...
    Private Sub ThreadStartTimer_kid(ByRef kida As testpu)
        ...
        Dim oWeelAuto As DoubleAnimation
        Dim oTransform As New RotateTransform
        ...
        If dTime > TimeSpan.FromSeconds(20) Then
            ...
            kida.ElementTime = DateAdd(DateInterval.Second, rand.Next(0, 30), Now)
            Try
                ...
                ElseIf sType = "animation" Then
                    If OSIsoft.AF.PI.PIPoint.TryFindPIPoint(srvAF, kida.ElementPITag, sinusoid) Then
                        ...
                        oWeelAuto = New DoubleAnimation
                        If kida.ElementTag = "clockwise" Then
                            oWeelAuto.From = -360
                        ElseIf kida.ElementTag = "counterclock" Then
                            oWeelAuto.From = 360
                        End If

                        oWeelAuto.Duration = New Duration(New TimeSpan(0, 0, 0, 5))
                        oWeelAuto.RepeatBehavior = New RepeatBehavior()
                        oWeelAuto.RepeatBehavior = RepeatBehavior.Forever

                        If element.SelectSingleNode("cursta").InnerText = "on" Then
                            'kida.ElementObject.RenderTransform = oTransform1
                            oTransform.BeginAnimation(RotateTransform.AngleProperty, oWeelAuto)
                            kida.ElementObject.RenderTransform = oTransform
                            imagePath = "/CDB_Overview;component/Images/wheel_bold_on.png"

                            If InStr(kida.ElementName, "fan") > 0 Then
                                imagePath = "/CDB_Overview;component/Images/fan_on.png"
                            End If
                        Else
                            ...
                            If InStr(kida.ElementName, "fan") > 0 Then
                                imagePath = "/CDB_Overview;component/Images/fan.png"
                            End If
                        End If
    ...
    Private Sub Scot_1_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
    ...
    Private Sub Scot_1_SizeChanged(sender As Object, e As SizeChangedEventArgs) Handles Me.SizeChanged
        Dim myscale As ScaleTransform
        Dim xxscale, yyscale As Single
